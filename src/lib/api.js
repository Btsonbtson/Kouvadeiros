...
async function call(method, path, body) {
  ...
  if (res.status === 401) {
    localStorage.removeItem('kouv_token'); localStorage.removeItem('kouv_user')
    window.location.reload(); throw new Error('Session expired')
  }
...
export const api = {
  ...
  changePassword: (currentPw, newPw)         => call('PATCH', '/change-password', { currentPw, newPw }),
  // Admin only
  adminSetPassword:(email, newPw)            => call('PATCH', '/admin-set-password', { email, newPw }),
  markChatRead:   ()                         => call('PATCH', '/chat-read',       {}),
}
