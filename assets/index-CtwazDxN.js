...
class ErrorBoundary extends reactExports.Component {
  ...
  componentDidCatch(e, info) {
    ...
    if (el2) el2.setAttribute("data-error", e.message);
  }
  render() {
    var _a, _b, _c;
    if (this.state.error) {
      const msg = String(((_a = this.state.error) == null ? void 0 : _a.message) || this.state.error);
      const stack = ((_c = (_b = this.state.error) == null ? void 0 : _b.stack) == null ? void 0 : _c.split("\n").slice(0, 2).join(" | ")) || "";
...
try {
  ...
  const el2 = document.getElementById("root");
  if (el2) el2.innerHTML = '<div style="min-height:100vh;background:#08090d;color:#ff4d6d;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:24px;font-family:monospace;gap:12px"><div style="font-size:28px">❌</div><div style="font-size:15px;font-weight:700;color:#fff">Boot Error</div><div style="font-size:11px;color:#ff8fa3;word-break:break-all;max-width:460px;background:rgba(255,77,109,.1);padding:14px;border-radius:10px">' + String(e) + "<br><br>" + (e.stack || "").split("\\n").slice(0, 3).join("<br>") + '</div><button onclick="location.reload()" style="background:#ff4d6d;color:#fff;border:none;border-radius:8px;padding:10px 24px;font-size:14px;font-weight:700;cursor:pointer;margin-top:8px">Ανανέωση</button></div>';
}
